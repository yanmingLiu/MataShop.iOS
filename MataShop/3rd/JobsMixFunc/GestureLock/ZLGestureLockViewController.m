...
- (void)createGesturesPassword:(NSMutableString *)gesturesPassword{
    ...
    if (self.lastGesturePsw.length == 0) {
        ...
        if (gesturesPassword.length < 4) {
            self.statusLabel.text = Internationalization(@"至少连接四个点，请重新输入");
        ...
        self.statusLabel.text = Internationalization(@"请再次绘制手势密码");
    ...
    if ([self.lastGesturePsw isEqualToString:gesturesPassword]) { // 绘制成功
        ...
        self.statusLabel.text = Internationalization(@"与上一次绘制不一致，请重新绘制");
        [self shakeAnimationForView:self.statusLabel];
        [self.gestureLockView clearLockView];
    }
}
...
- (void)validateGesturesPassword:(NSMutableString *)gesturesPassword {
    ...
    if ([gesturesPassword isEqualToString:[ZLGestureLockViewController gesturesPassword]]) {
        ...
        if (errorCount - 1 == 0) { // 你已经输错五次了！ 退出重新登陆！
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:Internationalization(@"手势密码已失效")
                                                                message:@"请重新登陆"
                                                               delegate:self
                                                      cancelButtonTitle:nil
                                                      otherButtonTitles:Internationalization(@"重新登陆"), nil];
            [alertView show];
            errorCount = 5;
            return;
        }
        
        self.statusLabel.text = [NSString stringWithFormat:Internationalization(@"密码错误，还可以再输入%ld次"),--errorCount];
        [self shakeAnimationForView:self.statusLabel];
        [self.gestureLockView clearLockView];
    }
}
...
-(UILabel *)nameLabel{
    if (!_nameLabel) {
        ...
        _nameLabel.text = Internationalization(@"smile丽");
...
-(UILabel *)statusLabel{
    if (!_statusLabel) {
        ...
        _statusLabel.text = Internationalization(@"请绘制手势密码");
...
-(UIButton *)otherAcountBtn{
    if (!_otherAcountBtn) {
        ...
        [_otherAcountBtn jobsBtnClickEventBlock:^(id data) {
                    NSLog(@"点击其他账号登陆按钮")
        }];
...
-(UIButton *)resetPswBtn{
    if (!_resetPswBtn) {
        ...
        @jobs_weakify(self)
        [_resetPswBtn jobsBtnClickEventBlock:^(id data) {
            @jobs_strongify(self)
...
-(UIButton *)forgetPswBtn{
    if (!_forgetPswBtn) {
        ...
        [_forgetPswBtn jobsBtnClickEventBlock:^(id data) {
            NSLog(@"点击忘记手势密码按钮");
        }];
