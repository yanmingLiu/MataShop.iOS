...
/**
 ...
 //    NSDictionary *headers = @{};
     ...
     [ZBRequestManager requestWithConfig:^(ZBURLRequest * _Nullable request) {
         ...
         request.url = [request.server stringByAppendingString:self.userInfoInviteFriendPOST.url];
...
+(void)postUploadImagePOST:(id)parameters
 ...
 [ZBRequestManager requestWithConfig:^(ZBURLRequest * request) {
     request.server = self.BaseUrl;
     request.url = [request.server stringByAppendingString:self.postUploadImagePOST.url];
     NSLog(@"request.URLString = %@",request.url);
...
-(void)networking_postuploadVideoPOST{
 ...
 +(void)postuploadVideoPOST:(id)parameters
     ...
     [ZBRequestManager requestWithConfig:^(ZBURLRequest * request) {
         request.server = self.BaseUrl;
         request.url = [request.server stringByAppendingString:self.postuploadVideoPOST.url];
         NSLog(@"request.URLString = %@",request.url);
         request.methodType = ZBMethodTypeUpload;
         request.apiType = ZBRequestTypeRefresh;//（默认为ZBRequestTypeRefresh 不读取缓存，不存储缓存）
 //        request.parameters = parameters;//与公共配置 Parameters 兼容
...
//    }
//}else{
//    [self jobsToastMsg:[@"异常接口" stringByAppendingString:NSObject.userInfoSelectVideoCountPOST.funcName]];
//}
