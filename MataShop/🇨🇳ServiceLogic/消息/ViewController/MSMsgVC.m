...
-(void)loadView{
    ...
    backImage.frame = CGRectMake(0, 0, JobsMainScreen_WIDTH(), self.view.height);
...
//    [self.calendar insertSubview:ImageView atIndex:0];
    ...
    _signBtn = signBtn;
    [signBtn setBackgroundColor:JobsYellowColor];
...
- (void)calendarConfig{
    ...
    //获取日历要显示的日期范围
    NSArray *timeArray = [MSMsgVC getStartTimeAndFinalTime];
...
//    self.calendar.appearance.adjustsFontSizeToFitContentSize = NO;
    ...
    //日历头部颜色
    self.calendar.calendarHeaderView.backgroundColor = JobsRedColor;
    self.calendar.calendarWeekdayView.backgroundColor = JobsRedColor;
    //载入节假日
