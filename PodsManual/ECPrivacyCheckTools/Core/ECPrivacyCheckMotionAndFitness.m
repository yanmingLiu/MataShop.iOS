...
+ (ECMotionAndFitnessAuthorizationStatus)motionAndFitnessAuthorizationStatus {
    if ([CMMotionActivityManager isActivityAvailable]) {
        ...
        } else return ECMotionAndFitnessAuthorizationStatusAuthorized;
    } else return ECMotionAndFitnessAuthorizationStatusUnable;
}
...
+ (void)requestMotionAndFitnessAuthorizationWithCompletionHandler:(void(^)(BOOL granted))completionHandler {
    if (@available(iOS 11.0, *)) {
        ...
        if (status == ECMotionAndFitnessAuthorizationStatusNotDetermined) {
            __block CMMotionActivityManager *cmManager = CMMotionActivityManager.new;
            ...
            [self callbackOnMainQueue:^{
                if (completionHandler) completionHandler(status == ECMotionAndFitnessAuthorizationStatusAuthorized);
            }];
        }
    } else {
